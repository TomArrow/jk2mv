...
void CL_InitUI(qboolean mainMenu) {
	...
	if ( (!mainMenu || mv_menuOverride->integer == 1) && mv_menuOverride->integer != -1 ) {
		...
		uivm = VM_Create(cl_uiName->string, (qboolean)!!mv_menuOverride->integer, CL_UISystemCalls, interpret);
	}
	...
	if (v != UI_API_VERSION && v != UI_API_VERSION_1_02) {
		CL_ShutdownUI();
		Com_Error(ERR_DROP, "User Interface is version %d, expected %d or %d", v, UI_API_VERSION, UI_API_VERSION_1_02);
	} else {
		int apireq;
