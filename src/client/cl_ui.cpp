...
*/
	...
	Com_Error( ERR_DROP, "Bad UI system trap: %" INT64PRINTF, (int64_t)args[0] );
	return 0;
}
...
void CL_InitUI(qboolean mainMenu) {
	...
	if ( (!mainMenu || mv_menuOverride->integer == 1) && mv_menuOverride->integer != -1 ) {
		...
		uivm = VM_Create("ui", (qboolean)(!!mv_menuOverride->integer || !!(com_basePathLibraries->integer & 4)), CL_UISystemCalls, interpret);
	}
