...
bool SVC_CrossServerCommand( netadr_t from ) {
	...
	if ( !strlen(sv_crossServerCommandPassword->string ) ) {
		...
		if (!Q_stricmp("s", Cmd_Argv(4))) {
			...
			const char* forward = va("csc \"%s\" %d %d c %s", sv_crossServerCommandPassword->string, serverUniqueCrossServerCommandsId, cscForwardingUniqueServerId, Cmd_ArgsFromQuoted(5));
			Cmd_DropArg(4);
			Cmd_DropArg(3);
			Cmd_DropArg(2);
			Cmd_DropArg(1);
...
#define sv_botFpsAllowIrregular sv_gameFpsAllowIrregular
	...
	if ( cvar_modifiedFlags & CVAR_SYSTEMINFO ) {
		char* systemInfo = Cvar_InfoString_Big( CVAR_SYSTEMINFO );
