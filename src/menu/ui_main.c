...
int MVAPI_Init(int apilevel) {
	if (!((int)trap_Cvar_VariableValue("mv_apienabled")) || apilevel < MV_APILEVEL) {
		...
		trap_Error(va("This mvmenu version requires JK2MV " MV_MIN_VERSION " (%i level).\n", apilevel));
	}
...
static void UI_BuildPlayerList() {
	...
	for( n = 0; n < count; n++ ) {
		...
		if (info[0]) {
			...
			Q_CleanStr( uiInfo.playerNames[uiInfo.playerCount], qfalse, qfalse );
			uiInfo.playerIndexes[uiInfo.playerCount] = n;
			uiInfo.playerCount++;
			team2 = atoi(Info_ValueForKey(info, "t"));
			if (team2 == team && n != uiInfo.playerNumber) {
				Q_strncpyz( uiInfo.teamNames[uiInfo.myTeamCount], Info_ValueForKey( info, "n" ), MAX_NAME_LENGTH );
				Q_CleanStr( uiInfo.teamNames[uiInfo.myTeamCount], qfalse, qfalse );
...
// modified by Sil
// added to joMME and modified by ent
...
static void UI_BuildFindPlayerList(qboolean force) {
	...
	if (!force) {
		...
		Q_CleanStr(uiInfo.findPlayerName, qfalse, qfalse);
