...
void		QDECL NET_OutOfBandPrint( netsrc_t net_socket, netadr_t adr, PRINTF_FORMAT_STRING const char *format, ...) __attribute__ ((format (printf, 3, 4)));
...
#define	NUM_SERVER_PORTS	30		// broadcast scan this many ports after
...
void 		QDECL Com_Printf( PRINTF_FORMAT_STRING const char *fmt, ... ) __attribute__ ((format (printf, 1, 2)));
void		QDECL Com_Printf_Ext( qboolean extendedColors, PRINTF_FORMAT_STRING const char *msg, ... ) __attribute__ ((format (printf, 2, 3)));
void		QDECL Com_Printf_MV( int flags, PRINTF_FORMAT_STRING const char *msg, ... ) __attribute__ ((format (printf, 2, 3)));
void 		QDECL Com_DPrintf( PRINTF_FORMAT_STRING const char *fmt, ... ) __attribute__ ((format (printf, 1, 2)));
void		QDECL Com_OPrintf( PRINTF_FORMAT_STRING const char *fmt, ...) __attribute__ ((format (printf, 1, 2))); // Outputs to the VC / Windows Debug window (only in debug compile)
Q_NORETURN void QDECL  Com_Error( errorParm_t code, PRINTF_FORMAT_STRING const char *fmt, ... ) __attribute__ ((format (printf, 2, 3)));
