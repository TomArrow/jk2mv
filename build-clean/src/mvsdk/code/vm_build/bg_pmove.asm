...
LABELV $2785
line 6894
;6894:	else if((!pm->requiredCmdMsec || pml.msec == pm->requiredCmdMsec) && (pm->isSpecialPredict || pm->modParms.msecRestrict <= 0 || pm->modParms.msecRestrict == pml.msec))
ADDRLP4 172
ADDRGP4 pm
INDIRP4
CNSTI4 456
...
;6895:	{
line 6896
;6896:		if (pm->modParms.physics == MV_CHARGEJUMP) {
ADDRGP4 pm
INDIRP4
CNSTI4 284
ADDP4
...
LABELV $2794
line 6899
;6899:		if (pm->ps->pm_flags & PMF_TIME_WATERJUMP) {
ADDRGP4 pm
INDIRP4
INDIRP4
CNSTI4 12
...
EQI4 $2796
line 6900
;6900:			PM_WaterJumpMove();
ADDRGP4 PM_WaterJumpMove
CALLV
pop
line 6901
;6901:		} else if ( pm->waterlevel > 1 ) {
ADDRGP4 $2797
JUMPV
LABELV $2796
ADDRGP4 pm
...
line 6916
;6914:
;6915:	// set groundentity, watertype, and waterlevel
;6916:	PM_GroundTrace();
ADDRGP4 PM_GroundTrace
CALLV
pop
line 6917
;6917:	PM_SetWaterLevel();
ADDRGP4 PM_SetWaterLevel
CALLV
pop
line 6919
;6918:
;6919:	PM_CheckRollEnd();
...
;6920:
;6921:
;6922:	if (pm->cmd.forcesel != (byte)-1 && (pm->ps->fd.forcePowersKnown & (1 << pm->cmd.forcesel)))
ADDRLP4 188
ADDRGP4 pm
INDIRP4
ASGNP4
...
;6923:	{
line 6924
;6924:		pm->ps->fd.forcePowerSelected = pm->cmd.forcesel;
ADDRLP4 196
ADDRGP4 pm
INDIRP4
ASGNP4
...
LABELV $2803
line 6926
;6926:	if (pm->cmd.invensel != (byte)-1 && (pm->ps->stats[STAT_HOLDABLE_ITEMS] & (1 << pm->cmd.invensel)))
ADDRLP4 196
ADDRGP4 pm
INDIRP4
ASGNP4
...
;6927:	{
line 6928
;6928:		pm->ps->stats[STAT_HOLDABLE_ITEM] = BG_GetItemIndexByTag(pm->cmd.invensel, IT_HOLDABLE);
ADDRLP4 204
ADDRGP4 pm
INDIRP4
ASGNP4
...
line 6937
;6935:
;6936:	// footstep events / legs animations
;6937:	PM_Footsteps();
...
line 6940
;6938:
;6939:	// entering / leaving water splashes
;6940:	PM_WaterEvents();
...
line 6951
;6941:
;6942:	//Walbug fix start, if getting stuck w/o noclip is even possible.  This should maybe be after round float? im not sure..
;6943:	// TODO MAYBE jaPRO this actually kills strafing on yavin. find better solution.
;6944:	//if ((pm->ps->persistant[PERS_TEAM] != TEAM_SPECTATOR) && pm->ps->stats[STAT_RACEMODE] && VectorCompare(pm->ps->origin, pml.previous_origin) /*&& (VectorLengthSquared(pm->ps->velocity) > VectorLengthSquared(pml.previous_velocity))*/)
;6945:	//	VectorClear(pm->ps->velocity); //Their velocity is increasing while their origin is not moving (wallbug), so prevent this..
;6946:		//VectorCopy(pml.previous_velocity, pm->ps->velocity);
;6947:	//To fix rocket wallbug, since that gets applied elsewhere, just always reset vel if origins dont match?
;6948:	//Wallbug fix end
;6949:
;6950:	// snap some parts of playerstate to save network bandwidth
;6951:	if (pm->ps->persistant[PERS_TEAM] == TEAM_SPECTATOR) {
ADDRGP4 pm
INDIRP4
INDIRP4
CNSTI4 292
...
NEI4 $2807
line 6952
;6952:		trap_SnapVector( pm->ps->velocity );
ADDRGP4 pm
INDIRP4
INDIRP4
CNSTI4 32
...
;6954:	else {
line 6955
;6955:		if (/*pm->modParms.raceMode || */pm->pmove_float > 2  && !pm->modParms.raceMode || pm->modParms.msecRestrict == -2) {
ADDRLP4 204
ADDRGP4 pm
INDIRP4
ASGNP4
...
LABELV $2822
line 6962
;6958:			&& !pm->modParms.raceMode
;6959:			&& (pml.msec <= 4 || pml.msec > 25)
;6960:			|| pm->requiredCmdMsec && pm->requiredCmdMsec != pml.msec
;6961:			|| !pm->isSpecialPredict && pm->modParms.msecRestrict > 0 && pm->modParms.msecRestrict != pml.msec
;6962:			) { //do nothing above 250FPS or below 40FPS, or if a certain msec timing is demanded by the game via requiredCmdMsec (used for toggle limiting via g_fpsToggleDelay)
line 6963
;6963:		}
ADDRGP4 $2814
JUMPV
LABELV $2813
line 6964
;6964:		else if (pm->pmove_float == 2 && !pm->modParms.raceMode) { //pmove_float 2: snaps vertical velocity only, so 125/142fps jumps are still the same height?
ADDRLP4 232
ADDRGP4 pm
INDIRP4
ASGNP4
...
NEI4 $2824
line 6966
;6965:			// TODO allow this option in racemode somehow too?
;6966:			vec3_t oldVelocity = { 0 };
ADDRLP4 236
ADDRGP4 $2826
INDIRB
ASGNB 12
line 6967
;6967:			VectorCopy( pm->ps->velocity, oldVelocity );
ADDRLP4 236
ADDRGP4 pm
INDIRP4
INDIRP4
...
ASGNB 12
line 6968
;6968:			trap_SnapVector( pm->ps->velocity );
ADDRGP4 pm
INDIRP4
INDIRP4
CNSTI4 32
...
pop
line 6969
;6969:			pm->ps->velocity[2] = oldVelocity[2];
ADDRGP4 pm
INDIRP4
INDIRP4
CNSTI4 40
...
LABELV $2824
line 6971
;6971:		else if (!pm->pmove_float || pm->modParms.raceMode && pm->modParms.msecRestrict > -2) {
ADDRLP4 236
ADDRGP4 pm
INDIRP4
ASGNP4
...
LABELV $2830
line 6972
;6972:			trap_SnapVector( pm->ps->velocity );
ADDRGP4 pm
INDIRP4
INDIRP4
CNSTI4 32
...
line 6981
;6980:
;6981:	if (pm->modParms.runFlags & RFL_BOT) {
ADDRGP4 pm
INDIRP4
CNSTI4 288
ADDP4
...
EQI4 $2833
line 6982
;6982:		pm->cmd.angles[ROLL] = oldCmdRoll;
ADDRGP4 pm
INDIRP4
CNSTI4 16
ADDP4
...
line 6995
;6985:
;6986://extern Q_INLINE int PM_GetRaceMode(pmove_t* pmove);
;6987:
;6988:/*
;6989:================
;6990:Pmove
;6991:
;6992:Can be called by either the server or the client
;6993:================
;6994:*/
;6995:void Pmove (pmove_t *pmove) {
line 6997
;6996:	int			finalTime;
;6997:	int			zeroahaha = 0.0f;
ADDRLP4 4
CNSTI4 0
ASGNI4
line 6999
;6998:
;6999:	PM_SetModData(pmove);
ADDRFP4 0
INDIRP4
ARGP4
...
line 7001
;7000:
;7001:	finalTime = pmove->cmd.serverTime;
ADDRLP4 0
ADDRFP4 0
INDIRP4
CNSTI4 4
...
line 7003
;7002:
;7003:	VectorCopy(pmove->ps->velocity,pmove->lastAntiLoopVelocity);
ADDRLP4 8
ADDRFP4 0
INDIRP4
ASGNP4
...
;7004:
;7005:#ifdef Q3_VM
;7006:	pmove->accelMiss =0.0f/0.0f; // putting NaN in there.
ADDRLP4 12
CNSTF4 0
ASGNF4
ADDRFP4 0
...
line 7011
;7007:#else
;7008:	pmove->accelMiss = 0.0f / zeroahaha; // putting NaN in there.
;7009:#endif
;7010:
;7011:	if (pmove->debugLevel> 30) {
ADDRFP4 0
INDIRP4
CNSTI4 68
ADDP4
...
line 7013
;7012:
;7013:		Com_Printf("accelmiss nonvalue is %f", pmove->accelMiss);
ADDRGP4 $2838
ARGP4
ADDRFP4 0
INDIRP4
...
line 7016
;7015:
;7016:	if ( finalTime < pmove->ps->commandTime ) {
ADDRLP4 0
INDIRI4
ADDRFP4 0
INDIRP4
...
;7018:	}
;7019:
;7020:	if ( finalTime > pmove->ps->commandTime + 1000 ) {
ADDRLP4 0
INDIRI4
ADDRFP4 0
INDIRP4
...
LEI4 $2841
line 7021
;7021:		pmove->ps->commandTime = finalTime - 1000;
ADDRFP4 0
INDIRP4
INDIRP4
ADDRLP4 0
...
line 7024
;7023:
;7024:	if (pmove->ps->fallingToDeath)
ADDRFP4 0
INDIRP4
INDIRP4
CNSTI4 1352
...
line 7032
;7031:
;7032:	pmove->ps->pmove_framecount = (pmove->ps->pmove_framecount+1) & ((1<<PS_PMOVEFRAMECOUNTBITS)-1);
ADDRLP4 16
ADDRFP4 0
INDIRP4
INDIRP4
...
line 7034
;7033:
;7034:	pmove->unalteredCmd = pmove->cmd; // so we can decide which direction to roll with strafebot (as roll overwrites keys that arent part of its direction)
ADDRLP4 20
ADDRFP4 0
INDIRP4
ASGNP4
...
line 7036
;7035:
;7036:	if (pmove->roll.status == ROLL_ENDED) {
ADDRFP4 0
INDIRP4
CNSTI4 476
ADDP4
...
NEI4 $2848
line 7037
;7037:		pmove->roll.status = ROLL_NONE;
ADDRFP4 0
INDIRP4
CNSTI4 476
ADDP4
...
line 7042
;7039:
;7040:	// chop the move up if it is too long, to prevent framerate
;7041:	// dependent behavior
;7042:	while ( pmove->ps->commandTime != finalTime ) {
line 7045
;7043:		int		msec;
;7044:
;7045:		msec = finalTime - pmove->ps->commandTime;
ADDRLP4 24
ADDRLP4 0
INDIRI4
ADDRFP4 0
...
;7052:		else {
line 7053
;7053:			qboolean isRaceMode = pmove->modParms.raceMode;
ADDRLP4 28
ADDRFP4 0
INDIRP4
CNSTI4 296
...
ASGNI4
line 7054
;7054:			if ( msec > 66 && !isRaceMode) { // if racemode, let other places handle this. we are not really concerned about someone gaining an undue advantage then. (is that even what its for? what even is the point since ppl can just send a bunch of shorter cmds at once)
ADDRLP4 24
INDIRI4
CNSTI4 66
LEI4 $2854
...
LABELV $2851
line 7058
;7058:		pmove->cmd.serverTime = pmove->ps->commandTime + msec;
ADDRLP4 28
ADDRFP4 0
INDIRP4
ASGNP4
...
line 7061
;7060:
;7061:		if ( pmove->ps->pm_flags & PMF_JUMP_HELD ) {
ADDRFP4 0
INDIRP4
INDIRP4
CNSTI4 12
