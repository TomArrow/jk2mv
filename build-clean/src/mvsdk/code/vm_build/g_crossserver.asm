code
proc G_CrossServerChatAll 5152 28
file "D:\Workspace\jk2mv-tom\src\mvsdk\code\game\g_crossserver.c"
line 6
;1:
;2:#include "g_local.h"
;3:#include "g_defrag.h"
;4:#include "g_dbcmds.h"
;5:
;6:static void G_CrossServerChatAll() {
line 15
;7:	int i;
;8:	gentity_t* other;
;9:	char	ourIdent[MAX_TOKEN_CHARS];
;10:	char	ourHostname[MAX_TOKEN_CHARS];
;11:	char	source[MAX_TOKEN_CHARS];
;12:	char	name[MAX_TOKEN_CHARS];
...
pop
line 17
;17:	trap_Argv(0, source, sizeof(source)); // 0 is ident
CNSTI4 0
ARGI4
ADDRLP4 2056
ARGP4
...
pop
line 18
;18:	if (!source[0] || !Q_stricmp(ourIdent,source)) { // use hostname if ident is identical to ours
ADDRLP4 2056
INDIRI1
CVII4 1
CNSTI4 0
...
;29:	else {
line 30
;30:		Q_StripColor(source);
ADDRLP4 2056
ARGP4
ADDRGP4 Q_StripColor
CALLV
pop
line 31
;31:		Com_sprintf(name, sizeof(name), "^l>^j%s^l>^7: ", source);
ADDRLP4 8
ARGP4
CNSTI4 1024
ARGI4
...
ASGNI4
line 34
;34:	trap_Argv(3, name + thelen, sizeof(name) - thelen);
CNSTI4 3
ARGI4
ADDRLP4 5128
INDIRI4
...
pop
line 35
;35:	trap_Argv(4, text, sizeof(text));
CNSTI4 4
ARGI4
ADDRLP4 1032
ARGP4
...
pop
line 39
;38:	// echo the text to the console
;39:	if (g_dedicated.integer) {
...
;44:	for (i = 0; i < level.maxclients; i++) {
ADDRLP4 0
CNSTI4 0
ASGNI4
ADDRGP4 $145
...
ASGNP4
line 46
;46:		G_SayTo(NULL, other, SAY_ALL, COLOR_GREEN, name, text, " crossServer");
CNSTP4 0
ARGP4
ADDRLP4 4
INDIRP4
...
line 49
;49:static void G_CrossServerPrint() {
line 52
;50:	char	firstArg[MAX_TOKEN_CHARS];
;51:	const char* reprint;
;52:	trap_Argv(3,firstArg,sizeof(firstArg));
CNSTI4 3
ARGI4
ADDRLP4 0
ARGP4
...
pop
line 54
;53:	//reprint = va("print \"cross-server: %s\" %s", firstArg,ConcatArgsQuoted(4));
;54:	reprint = va("print \"%s\" %s", firstArg,ConcatArgsQuoted(4));
CNSTI4 4
ARGI4
ADDRLP4 1028
...
ASGNP4
line 55
;55:	trap_SendServerCommand(-1, reprint);
CNSTI4 -1
ARGI4
ADDRLP4 1024
INDIRP4
...
line 58
;57:
;58:qboolean G_CrossServerCommand() {
line 77
;59:
;60:	char	cmd[MAX_TOKEN_CHARS];
;61:	int i, argc;
;62:	//char token[BIG_INFO_STRING]; // As the engine uses Cmd_TokenizeString2 a single parameter is theoretically not limited by MAX_TOKEN_CHARS, but by BIG_INFO_STRING
;63:
;64:	// Filter '\n' and '\r'
;65:	//argc = trap_Argc();
;66:	//for (i = 0; i < argc; i++)
;67:	//{
;68:	//	trap_Argv(i, token, sizeof(token));
;69:	//	if (strchr(token, '\n') || strchr(token, '\r'))
;70:	//	{
;71:	//		G_Printf("G_CrossServerCommand: got an invalid command - command blocked.\n");
;72:	//		return;
;73:	//	}
;74:	//}
;75:
;76:	//trap_Argv(0, cmd, sizeof(cmd));
;77:	trap_Argv(2, cmd, sizeof(cmd)); // 0 is serverident and 1 is sv_hostname
CNSTI4 2
ARGI4
ADDRLP4 0
ARGP4
...
;87:}
;88:
;89:void G_SendCrossServerCommand(const char* cmd) {
line 90
;90:	if (!(coolApi & COOL_APIFEATURE_CROSS_SERVER_COMMANDS)) {
ADDRGP4 coolApi
INDIRI4
CNSTI4 1048576
BANDI4
...
INDIRP4
ARGP4
ADDRGP4 trap_G_COOL_API_CrossServerCommand
CALLV
pop
line 94
;94:}
